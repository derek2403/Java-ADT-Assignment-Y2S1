...
const DonorList = () => {
  ...
  const filteredDonors = donors.filter(donor => 
    donor.type.toLowerCase().includes(typeFilter.toLowerCase()) &&
    donor.category.toLowerCase().includes(categoryFilter.toLowerCase())
  );
  ...
  const renderTable = () => {
    ...
    if (filteredDonors.length === 0) {
      return <p style={styles.noResults}>No matching donors found.</p>;
    }
